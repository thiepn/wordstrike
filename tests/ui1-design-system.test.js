...
test("UI1 stylesheet loads after legacy CSS so staged migration is deterministic", () => {
  const legacy = index.indexOf('href="style.css"');
  const uiSystem = index.indexOf('href="styles/ui-system.css"');
  assert.ok(legacy >= 0, "legacy stylesheet link is missing");
  assert.ok(uiSystem > legacy, "UI system must load after legacy style.css");
});
...
test("UI1 separates ordinary interface typography from gameplay/data typography", () => {
  ...
  for (const gameplaySelector of [
    ".word-visual",
    ".speed-test-word",
    ".hud-value",
    ".boss-timer",
    ".boss-phrase",
    ".arcade-rush-hud-stat strong",
  ]) {
...
test("UI1 includes the reusable primitive families needed by UI2–UI12", () => {
  for (const primitive of requiredPrimitives) {
    assert.ok(css.includes(primitive), `missing reusable primitive ${primitive}`);
  }
});
...
console.log("UI1 design-system tokens, primitives, typography, accessibility and Practice Lab boundary passed.");
