...
const requiredTokens = ["--font-ui", "--font-game", "--text-display", "--text-stat", "--color-bg", "--color-surface-1", "--color-surface-2", "--color-text-primary", "--color-text-muted", "--color-border-subtle", "--color-accent", "--color-special", "--color-danger", "--color-success", "--space-1", "--space-8", "--radius-md", "--shadow-elevation-medium", "--bloom-accent-sm", "--motion-fast", "--motion-cinematic", "--focus-ring"];
const requiredPrimitives = [".ui-surface", ".ui-button", ".ui-button--primary", ".ui-button--secondary", ".ui-button--ghost", ".ui-button--danger", ".ui-icon-button", ".ui-icon", ".ui-field", ".ui-input", ".ui-select", ".ui-toggle", ".ui-check", ".ui-segmented", ".ui-tabs", ".ui-metric", ".ui-badge", ".ui-backdrop", ".ui-modal", ".ui-hint", ".ui-tooltip", ".ui-hud", ".ui-hud-group", ".ui-hud-surface"];
...
test("UI1 separates ordinary interface typography from gameplay/data typography", () => {
  ...
  for (const selector of [".word-visual", ".speed-test-word", ".hud-value", ".boss-timer", ".boss-phrase", ".arcade-rush-hud-stat strong"]) assert.ok(css.includes(selector), `game/data font contract missing ${selector}`);
...
test("action interactions are stable, accessible and avoid legacy sideways motion", () => {
  assert.ok(!css.includes("translateX(3px)"));
  assert.ok(!/transition\s*:\s*all/i.test(css));
...
test("UI1 documentation freezes the neon semantic budget and later phase ownership", () => {
  for (const phrase of ["Neon communicates state", "80–90% neutral", "UI2 — Title + global navigation", "UI12 — final motion/audio/responsive/accessibility polish"]) assert.ok(designDoc.includes(phrase), `design source of truth missing: ${phrase}`);
});
