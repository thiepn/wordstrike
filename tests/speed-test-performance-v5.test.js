...
function word(expected, {
  ...
  backspaces = 0,
  wordDeletes = 0,
  clean = errors === 0 && backspaces === 0 && wordDeletes === 0,
} = {}) {
  return {
    expected,
    typed: expected,
...
function sample({ sessionId, endedAt, wpm, cleanAlpha = false, alphaWpm = 45, mistakeCount = 1 }) {
  const words = [
    word("alpha", {
      wpm: alphaWpm,
      errors: cleanAlpha ? 0 : 1,
      backspaces: cleanAlpha ? 0 : 1,
      clean: cleanAlpha,
    }),
    word("beta", { wpm: wpm + 4 }),
    word("gamma", { wpm: wpm + 2 }),
    word("delta", { wpm: wpm - 1 }),
  ];
  ...
  return {
    ...
    endedAt,
    wpm,
    accuracy: cleanAlpha ? 100 : 97,
    rawWpm: wpm + 6,
    wordCount: words.length,
    cleanWords,
    cleanPercent: (cleanWords / words.length) * 100,
    corrections,
    correctionsPerWord: corrections / words.length,
...
console.log("Typing Performance Timeline V5 historical baseline, persistent-friction, confusion, and ranked-data isolation passed.");
