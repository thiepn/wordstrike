...
const letters = (index) => { let value = index + 1; let result = ""; while (value) { value -= 1; result = String.fromCharCode(97 + value % 26) + result; value = Math.floor(value / 26); } return `word${result}`; };
...
const reference = { referenceId: "WS-COMMON-EN-1", referenceVersion: 1, words: Array.from({ length: 1200 }, (_, index) => ({ lexicalKey: letters(index), rank: index + 1, band: bandFor(index + 1) })) };
...
const stat = (key, opportunities, sessions = 1, lastObservedAt = "2026-09-01T00:00:00.000Z", extras = {}) => ({ recordVersion: 3, statId: `s_${key}`, profileId: "p", contextId: "c", entityType: "word", entityKey: key, lastObservedAt, evidence: { opportunities: { count: opportunities }, observation: { sessionCount: sessions, lastObservedAt } }, ...extras });
...
test("PL28 generic record schemas remain stable inside the current PL33 DB10 envelope", () => {
...
test("typing breadth uses canonical denominators and PL11/PL15-derived evidence states", () => {
  ...
  const masterySnapshot = { entities: [ { entityType: "word", entityKey: a.lexicalKey, automaticity: { score: 80, confidenceLevel: "medium" } }, { entityType: "word", entityKey: s.lexicalKey, automaticity: { score: 94, confidenceLevel: "high" } } ] };
  ...
  assert.equal(snapshot.bands.core.totalWords, 100); assert.equal(snapshot.overall.totalWords, 1200);
...
test("band metrics keep first-pass word correctness separate from launch and internal timing", () => {
  const contentPlan = { units: [ { type: "word", startIndex: 0, endIndex: 2, text: "ab", metadata: { commonWords: { band: "core" } } }, { type: "word", startIndex: 3, endIndex: 5, text: "cd", metadata: { commonWords: { band: "broad" } } } ] };
  const eventTrace = [ { type: "character", isFirstAttempt: true, textPosition: 0, expected: "a", correctness: "correct" }, { type: "character", isFirstAttempt: true, textPosition: 1, expected: "b", correctness: "incorrect" }, { type: "character", isFirstAttempt: true, textPosition: 3, expected: "c", correctness: "correct" }, { type: "character", isFirstAttempt: true, textPosition: 4, expected: "d", correctness: "correct" } ];
  const normalizedTransitions = [ { isFirstAttempt: true, textPosition: 0, latencyClass: "fluent", residualLatencyMs: 10 }, { isFirstAttempt: true, textPosition: 1, latencyClass: "disfluent", residualLatencyMs: 50 }, { isFirstAttempt: true, textPosition: 3, latencyClass: "disfluent", residualLatencyMs: 40 }, { isFirstAttempt: true, textPosition: 4, latencyClass: "fluent", residualLatencyMs: 20 } ];
  ...
  assert.equal(metrics.core.wordOpportunityCount, 1); assert.equal(metrics.core.wholeWordFirstPassAccuracy, 0); assert.equal(metrics.core.launchResidualMedianMs, 10); assert.equal(metrics.core.internalResidualMedianMs, null); assert.equal(metrics.broad.wholeWordFirstPassAccuracy, 1); assert.equal(metrics.broad.launchDisfluencyRate, 1); assert.equal(metrics.broad.internalResidualMedianMs, 20); assert.equal("wpm" in metrics.broad, false);
});
...
test("PL28 standardized Check contract remains exactly 200 words and 50 per band", () => { assert.equal(PRACTICE_COMMON_WORD_CHECK_WORD_COUNT, 200); assert.equal(PRACTICE_COMMON_WORD_CHECK_WORDS_PER_BAND, 50); });
