...
const mutatedDuplicatePayload = payload("session-outbox-first-0001");
mutatedDuplicatePayload.result.score = 999999;
const duplicate = enqueueSubmissionOutbox(
  "endless",
  mutatedDuplicatePayload,
  "user-1",
  { storage, now: 1200 },
);
assert.equal(duplicate.ok, true);
assert.equal(duplicate.entry.createdAt, 1000);
assert.equal(duplicate.entry.immutablePayload.result.score, 1,
  "re-enqueueing the same session must not rewrite the durable payload");
