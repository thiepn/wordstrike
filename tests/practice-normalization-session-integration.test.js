...
test("PL10 generic normalization remains canonical inside the current PL16 session envelope", async () => {
...
test("PL10 experiment analyzers receive frozen PL16 foundationAnalysis v7 and cannot own normalization or canonical skill evidence", async () => {
  let received = null;
  ...
  await typeCorrect(engine, harness, 26);
  const result = await engine.complete("manual-stop");
  assert.equal(received.foundationAnalysis.version, 7);
