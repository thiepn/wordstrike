...
test("unowned navigation keys remain native", () => {
...
test("Arcade Rush ready and results routes use injected high-level actions only", () => {
  ...
  const focusedMainMenu = createHarness({
    screen: Screens.ARCADE_RUSH_RESULTS,
    arcadeRushResultsReadyAt: 0,
    arcadeRushResultsIndex: 0,
  });
  focusedMainMenu.handle(eventFor("ArrowDown", {
    target: {
      tagName: "BUTTON",
      matches: () => false,
  ...
  assert.equal(focusedMainMenu.state.arcadeRushResultsIndex, 3);
  assert.deepEqual(focusedMainMenu.calls, [["sync-result", Screens.ARCADE_RUSH_RESULTS, 3]]);
});
