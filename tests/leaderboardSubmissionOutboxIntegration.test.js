...

{
  ...
  let resolveRequest;
  const service = createLeaderboardSubmissionService({
    getClient: () => ({ functions: { invoke: () => new Promise((resolve) => {
      resolveRequest = resolve;
    }) } }),
    enqueueOutbox: () => ({ ok: true, error: null, entry: {} }),
    markOutboxAttempt: () => true,
    removeOutbox: (sessionId, userId) => {
  ...
  service.refreshSubmissionEligibility(
    { status: "signed-in", user: { id: "user-2" } },
    { status: "ready", profile: { username: "Player_2" } },
  );
  resolveRequest({ data: { ok: true, data: { duplicate: false, rank: 8 } }, error: null });
  await pending;
  assert.deepEqual(removals, [[SESSION, "user-1"]],
    "a mid-request account change must not remove another user's outbox entry");
}

console.log("Leaderboard submission persists retry intent before network, retains failures, survives account changes, and clears durable intent after success.");
