...
def certify_v4_hardening(browser, browser_name, base, evidence):
    ...
    # A syntactically valid but retired/unknown theme must never leak into the
    # public release route or planner.
    assert plan["theme"] == "mixed", plan
    ...
    # Jump across the first document boundary in one beforeinput transaction.
    # This certifies that source identity follows the active stream segment even
    # under paste/multi-character input, without a stale header.
    ...
    # Repeated rerolls must not re-arm the bootstrap observer or create a setup
    # surface. This is a compact lifecycle/long-session stress probe.
    for _ in range(12):
        page.keyboard.press("Tab")
        expect(page.locator('[data-flow-view="run"]')).to_be_visible(timeout=10000)
        lifecycle = page.evaluate("window.wordstrikeFlowPhase1.getLifecycleDiagnostics()")
...
def certify_offline(browser, browser_name, base, evidence):
    ...
    cached = page.evaluate("""async () => {
      const targets = [
        './js/flow/flowRuntimeLoader.js?v=20260924h',
        './js/flow/flowPhase1.js?v=20260924g',
