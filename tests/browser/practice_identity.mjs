...
try{for(const width of widths){
 ...
 try{
  ...
  assert.equal(await page.locator('.pl-navigation [data-route="research"]').count(),0,'Research stays out of the normal public Practice navigation');
  for(const id of ['read-ahead','metronome-typing']){const card=page.locator(`[data-lab-drill="${id}"]`);assert.equal(await card.locator('.pl-drill-status').getAttribute('data-status'),'available',`${id} must be truly runnable after graduation`);assert.doesNotMatch(await card.innerText(),/Experimental|Unavailable/i);}
  assert.doesNotMatch(await page.locator('[data-lab-drill="weakness-boss"]').innerText(),/Experimental/i);
  await noOverflow('home');await shot('home');if(width===1440||width===390)await audit('home');
  const help=page.locator('.practice-lab-help');assert.equal(await help.isEnabled(),true,'Practice Help must be available');await help.focus();await page.keyboard.press('Enter');const helpDialog=page.locator('.onboarding-dialog[data-tutorial-id="practice"]');await helpDialog.waitFor({state:'visible'});assert.equal(await helpDialog.getAttribute('role'),'dialog');assert.equal(await helpDialog.getAttribute('aria-modal'),'true');assert.match(await helpDialog.innerText(),/Full Assessment is optional/);await page.keyboard.press('Escape');await helpDialog.waitFor({state:'detached'});assert.ok(await help.evaluate(e=>e===document.activeElement),'Closing Practice Help restores focus');
