...
try{for(const width of widths){
 ...
 try{
  ...
  for(const id of ids){await page.locator(`[data-lab-drill="${id}"] button`).click();await page.locator('.pl-navigation').waitFor();await page.locator('.practice-lab-detail h1').first().waitFor();await noOverflow(id);record.setups.push({id,title:await page.locator('.practice-lab-screen h1').first().innerText()});if(['weak-keys','custom-text','full-assessment','numbers-symbols'].includes(id))await shot(`${id}-setup`);assert.equal(await page.locator('.pl-method-note input,.pl-method-note button,.pl-method-note select,.pl-method-note textarea').count(),0,'Setup controls are never collapsed');await home();}
