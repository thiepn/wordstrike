...
try{for(const width of (process.env.PRACTICE_WIDTHS??'1440,390,320').split(',').map(Number)){
 ...
 try{
  ...
  assert.deepEqual(undersized,[],'Letter keys are at least 44px');await shot('letter-setup');await audit('letter setup');
  ...
  await page.getByRole('button',{name:'Compact list view',exact:true}).click();await page.locator('[data-lab-filter="speed"]').click();assert.equal(await page.locator('[data-lab-drill]:visible').count(),2);
  await page.locator('#practice-catalog-title').scrollIntoViewIfNeeded();await shot('compact-library');await audit('compact library');
  await page.locator('[data-lab-drill="pace-ladder"] button').click();await home();assert.equal(await page.locator('.pl-studio-home').getAttribute('data-lab-layout'),'list');assert.equal(await page.locator('[data-lab-filter="speed"]').getAttribute('aria-pressed'),'true');
