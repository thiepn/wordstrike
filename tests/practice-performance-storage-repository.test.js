...
test("PL14 advances only DB/session envelope and DB3->DB4 creates performanceStates only", () => {
...
test("PL14 historical session v7 migrates sequentially to v8 with null performance measurement", () => {
  ...
  assert.equal(migration.toVersion, 8);
  assert.deepEqual(migration.steps, ["sessionSummary:7->8"]);
