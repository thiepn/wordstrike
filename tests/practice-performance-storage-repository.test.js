...
test("PL14 performance-state contract remains intact inside the current Practice DB/session envelope", () => {
...
test("PL14 historical session v7 performance migration remains null through the current wrapper", () => {
  ...
  assert.equal(migration.toVersion, PRACTICE_RECORD_VERSIONS.sessionSummary);
  assert.deepEqual(migration.steps, ["sessionSummary:7->8", "sessionSummary:8->9", "sessionSummary:9->10", "sessionSummary:10->11", "sessionSummary:11->12", "sessionSummary:12->13", "sessionSummary:13->14"]);
