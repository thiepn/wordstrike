...
test("PL10 model versions remain stable inside the current PL14 storage/session/foundation envelope", () => {
...
test("PL10 sessionSummary v4 migration preserves null normalization evidence through current PL14 v8", () => {
  ...
  assert.equal(migrated.toVersion, 8);
  assert.deepEqual(migrated.steps, ["sessionSummary:4->5", "sessionSummary:5->6", "sessionSummary:6->7", "sessionSummary:7->8"]);
...
test("PL10 normalization remains null in the complete historical v1 -> v8 migration chain", () => {
  const current = createDefaultSessionSummary();
