...
const [index, appCss, presentationBootstrap, presentationLifecycle, css, presentation, audio, storage, designSystem, modes, workflow] = await Promise.all([
...
assert.match(presentationBootstrap, /syncProfileLeaderboardsSettingsPresentation[\s\S]*syncUi12GlobalPresentation[\s\S]*syncTypingResultsRuntime/);
assert.match(presentationBootstrap, /\{ id: "ui12-global", sync: syncUi12GlobalPresentation \}/);
...
assert.doesNotMatch(presentation, /new MutationObserver/,
  "UI12 presentation must share the V10 lifecycle instead of self-observing");
assert.match(presentation, /export function syncUi12GlobalPresentation\(/);
...
console.log("UI12 source contracts passed: semantic resource ownership, shared presentation lifecycle, opt-in lazy audio, reduced-motion polish, authoritative save integration, and Practice isolation.");
