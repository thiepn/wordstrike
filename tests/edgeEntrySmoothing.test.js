...
const currentNode = text.children[1];
const remainingNode = text.children[2];
const initialTextWrites = typedNode.textWrites + currentNode.textWrites + remainingNode.textWrites;
rendered.x = 20;
updateWordElement(rendered, false);
assert.equal(visual.innerHTMLWrites, 0);
assert.equal(typedNode.textWrites + currentNode.textWrites + remainingNode.textWrites, initialTextWrites);
...
assert.ok(typedNode.textWrites + currentNode.textWrites + remainingNode.textWrites > initialTextWrites);
...
assert.equal(currentNode._textContent, "a");
assert.equal(remainingNode._textContent, "mple");
updateWordElement(rendered, false, { candidate: false, prefixLength: 0 });
assert.equal(typedNode._textContent, "");
assert.equal(currentNode._textContent, "e");
assert.equal(remainingNode._textContent, "xample");
clearWordElements();
