...
assert.equal(auth.experimental?.appendPkceFlowIdToRedirects, true,
  "Google OAuth callbacks must retain the verifier for the flow that initiated them");

const index = readFileSync(new URL("../index.html", import.meta.url), "utf8");
assert.match(index, /@supabase\/supabase-js@2\.116\.0/,
  "Production must load the certified Supabase browser SDK");
assert.match(index, /js\/main\.js\?v=20260910d/,
  "Auth-facing application code should be cache-busted after the login repair");

console.log("Google OAuth client uses current Supabase JS with per-flow PKCE verifier correlation.");
