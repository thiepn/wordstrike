...
set display_name = 'Flow',
    mode = 'flow',
    rules_version = 3,
    ranking_strategy = 'score_desc_accuracy_desc_wpm_desc_submitted_at_asc',
    is_active = true,
    is_visible = true
where board_key = 'flow-standard-v1';

update public.leaderboard_boards
set is_active = false,
    is_visible = false
where board_key in ('flow-quick-v1', 'flow-long-v1');
...
begin
    ...
    with eligible as (
        ...
        where s.board_key = selected_board.board_key
          ...
          and (selected_board.board_key <> 'flow-standard-v1' or s.completed = true)
    ),
    player_best as (
        select * from (
            select eligible.*,
                row_number() over (
                    ...
                    order by
                        ...
                        case when selected_board.board_key = 'flow-standard-v1' then score end desc nulls last,
                        case when selected_board.board_key = 'flow-standard-v1' then accuracy end desc nulls last,
                        case when selected_board.board_key = 'flow-standard-v1' then wpm end desc nulls last,
        ...
        'entries', coalesce((
            select jsonb_agg((
                ...
                || case
                    ...
                    when selected_board.board_key = 'flow-standard-v1'
