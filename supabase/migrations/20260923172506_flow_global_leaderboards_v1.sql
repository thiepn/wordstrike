...
alter table public.leaderboard_boards
    ...
    check (mode in ('daily_strike', 'endless', 'campaign', 'typing_test', 'arcade_rush', 'flow'));

insert into public.leaderboard_boards (
    board_key, display_name, mode, rules_version, ranking_strategy, is_active, is_visible
) values
    ('flow-quick-v1', 'Flow Quick', 'flow', 2, 'score_desc_accuracy_desc_wpm_desc_submitted_at_asc', true, true),
    ('flow-standard-v1', 'Flow Standard', 'flow', 2, 'score_desc_accuracy_desc_wpm_desc_submitted_at_asc', true, true),
    ('flow-long-v1', 'Flow Long', 'flow', 2, 'score_desc_accuracy_desc_wpm_desc_submitted_at_asc', true, true)
...
create index if not exists leaderboard_submissions_flow_rank_idx
    ...
    where board_key in ('flow-quick-v1', 'flow-standard-v1', 'flow-long-v1')
      and completed = true
...
begin
    ...
    with eligible as (
        select
            ...
            s.words_completed,
            case
                when jsonb_typeof(s.metrics -> 'consistency') = 'number'
                    then (s.metrics ->> 'consistency')::numeric
                else null
            end as consistency,
            s.submitted_at
        ...
        where s.board_key = selected_board.board_key
          ...
          and (selected_board.board_key not like 'flow-%' or s.completed = true)
    ),
    player_best as (
        select * from (
            select eligible.*,
                row_number() over (
                    ...
                    order by
                        ...
                        case when selected_board.board_key like 'flow-%' then score end desc nulls last,
                        case when selected_board.board_key like 'flow-%' then accuracy end desc nulls last,
                        case when selected_board.board_key like 'flow-%' then wpm end desc nulls last,
        ...
        'entries', coalesce((
            select jsonb_agg((
                ...
                || case
                    ...
                    when selected_board.board_key like 'flow-%'
