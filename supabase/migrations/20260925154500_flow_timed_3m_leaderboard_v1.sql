...
-- Timed Flow is a different competitive format from the retired unlimited
-- Flow V3 board. Keep old submissions intact for audit/history, but never mix
-- them into the new fixed-duration ranking.
...
insert into public.leaderboard_boards (
    ...
    'flow',
    4,
    'score_desc_accuracy_desc_wpm_desc_submitted_at_asc',
    true,
    true
)
...
create index if not exists leaderboard_submissions_flow_3m_rank_idx
    ...
    where board_key = 'flow-standard-3m-v1'
      and completed = true
...
begin
    ...
    with eligible as (
        ...
        where s.board_key = selected_board.board_key
          ...
          and (selected_board.board_key <> 'flow-standard-3m-v1' or s.completed = true)
    ),
    player_best as (
        select * from (
            select eligible.*,
                row_number() over (
                    ...
                    order by
                        ...
                        case when selected_board.board_key = 'flow-standard-3m-v1' then score end desc nulls last,
                        case when selected_board.board_key = 'flow-standard-3m-v1' then accuracy end desc nulls last,
                        case when selected_board.board_key = 'flow-standard-3m-v1' then wpm end desc nulls last,
        ...
        'entries', coalesce((
            select jsonb_agg((
                ...
                || case
                    ...
                    when selected_board.board_key = 'flow-standard-3m-v1'
