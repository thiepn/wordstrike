...
const weaknessBoss = Object.freeze({
  ...
  category: "advanced", iconKey: "target", accentKey: "precision", status: "preview", requiresAssessment: false, requiresPracticeData: true, difficulty: "adaptive",
...
const paceLongDescription = "A reference segment followed by eight controlled pace rungs that map where typing control begins to deteriorate. PL14 owns frontier inference; the guide is pace-position only and no live actual WPM or metronome is used.";
const burstLongDescription = "Six short typing sprints with preview and recovery intervals, used to estimate repeatable brief upper-speed capacity. A 30-second warm-up precedes the six bouts; PL13 receives at most one robust session observation and best single sprint remains descriptive.";

export const PRACTICE_EXPERIMENT_CATALOG = Object.freeze([
  ...CATALOG_V30.map((entry) => {
    if (entry.id === "pace-ladder") return Object.freeze({ ...entry, description: "A reference segment followed by eight controlled pace rungs that map where typing control begins to deteriorate.", longDescription: paceLongDescription, capabilities: Object.freeze(["target-blind", "fixed-protocol", "control-frontier", "pace-guide", "eight-rung-protocol"]) });
    if (entry.id === "burst-sprints") return Object.freeze({ ...entry, description: "Six short typing sprints with preview and recovery intervals, used to estimate repeatable brief upper-speed capacity.", longDescription: burstLongDescription, estimatedDurationMinutes: Object.freeze({ minimum: 3, recommended: 4, maximum: 4 }), capabilities: Object.freeze(["target-blind", "six-sprint-protocol", "warmup", "preview-intervals", "recovery-intervals", "burst-ability", "robust-top-three"]) });
