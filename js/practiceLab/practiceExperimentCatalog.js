// Phase 8 canonical Practice experiment catalog.
// The historical V30 baseline is embedded here so production has one catalog owner.

import * as __catalog_base_dep0 from "./practiceValidation.js";

const __catalog_base = (() => {
  const { validatePracticeSerializable } = __catalog_base_dep0;
  const PRACTICE_EXPERIMENT_STATUSES = Object.freeze(["planned", "available", "preview", "disabled", "hidden"]);
  const PRACTICE_EXPERIMENT_DIFFICULTIES = Object.freeze(["beginner", "intermediate", "advanced", "adaptive", "all-levels"]);
  const PRACTICE_EXPERIMENT_CATEGORIES = Object.freeze(["assessment", "precision", "speed", "fluency", "real-world", "advanced", "custom"]);
  const PRACTICE_CATEGORY_LABELS = Object.freeze({
    assessment: "Assess", precision: "Precision", speed: "Speed", fluency: "Fluency",
    "real-world": "Real-world", advanced: "Advanced", custom: "Custom",
  });
  const PRACTICE_ICON_KEYS = Object.freeze(["assessment", "key", "combination", "word", "target", "bolt", "words", "text", "wave", "metronome", "eye", "endurance", "punctuation", "numbers", "custom"]);
  const PRACTICE_ACCENT_KEYS = Object.freeze(["assessment", "precision", "speed", "fluency", "real-world", "custom"]);
  ...
  const PRACTICE_EXPERIMENT_IDS = Object.freeze(IDS);
  ...
  function validatePracticeExperimentCatalog(catalog) {
    const errors = [];
    const seen = new Set();
  ...
  const PRACTICE_EXPERIMENT_CATALOG = deepFreeze(definitions.map(buildEntry));
  ...
  const getPracticeExperiment = (experimentId) => CATALOG_BY_ID.get(experimentId) || null;
  
  const PRACTICE_DAILY_TRAINING = deepFreeze({
  ...
  return Object.freeze({ validatePracticeExperimentCatalog, PRACTICE_EXPERIMENT_STATUSES, PRACTICE_EXPERIMENT_DIFFICULTIES, PRACTICE_EXPERIMENT_CATEGORIES, PRACTICE_CATEGORY_LABELS, PRACTICE_ICON_KEYS, PRACTICE_ACCENT_KEYS, PRACTICE_EXPERIMENT_IDS, PRACTICE_EXPERIMENT_CATALOG, getPracticeExperiment, PRACTICE_DAILY_TRAINING });
})();
...
const __catalog_current = (() => {
  ...
  const METRONOME_CAPABILITIES = Object.freeze(["duration-options", "self-calibrated-fixed-tempo", "audio-with-visual-fallback", "counterbalanced-pulse-silent", "target-blind", "same-session-profile"]);
  ...
  const metronomeLongDescription = "A target-blind cadence practice protocol. A silent natural baseline selects one fixed tempo for the session, then counterbalanced pulse-on and pulse-off blocks compare pace variation, effective pace, first-pass accuracy, disfluency, and correction cost. The pulse is an external cadence cue, not a one-key-per-beat target.";
  const READ_AHEAD_CAPABILITIES = Object.freeze(["duration-options", "visual-preview-manipulation", "target-blind", "same-session-profile"]);
  const readAheadDescription = "Practice typing while WordStrike changes how many upcoming words remain visible. The session compares visible-preview conditions descriptively; it does not track your eyes.";
  const readAheadLongDescription = "A visual-preview exercise that keeps the current text visible while varying whether one, two, or four future words can be seen. It describes how visible preview changes typing performance in the session; it does not track gaze or eye movements.";
  ...
  const weaknessBoss = Object.freeze({
    ...
    capabilities: WEAKNESS_BOSS_CAPABILITIES, tags: Object.freeze(["adaptive", "weakness", "fixed-dose"]), primarySkill: "current limiter practice", implementationPrompt: 37, displayOrder: 60,
  });
  ...
  const PRACTICE_EXPERIMENT_CATALOG = Object.freeze([
  ...
  const getPracticeExperiment = (experimentId) => BY_ID.get(experimentId) ?? null;
  return Object.freeze({ PRACTICE_EXPERIMENT_CATALOG, PRACTICE_EXPERIMENT_IDS, getPracticeExperiment });
})();

export const validatePracticeExperimentCatalog = __catalog_base.validatePracticeExperimentCatalog;
export const PRACTICE_EXPERIMENT_STATUSES = __catalog_base.PRACTICE_EXPERIMENT_STATUSES;
export const PRACTICE_EXPERIMENT_DIFFICULTIES = __catalog_base.PRACTICE_EXPERIMENT_DIFFICULTIES;
export const PRACTICE_EXPERIMENT_CATEGORIES = __catalog_base.PRACTICE_EXPERIMENT_CATEGORIES;
export const PRACTICE_CATEGORY_LABELS = __catalog_base.PRACTICE_CATEGORY_LABELS;
export const PRACTICE_ICON_KEYS = __catalog_base.PRACTICE_ICON_KEYS;
export const PRACTICE_ACCENT_KEYS = __catalog_base.PRACTICE_ACCENT_KEYS;
export const PRACTICE_EXPERIMENT_IDS = __catalog_current.PRACTICE_EXPERIMENT_IDS;
export const PRACTICE_EXPERIMENT_CATALOG = __catalog_current.PRACTICE_EXPERIMENT_CATALOG;
export const getPracticeExperiment = __catalog_current.getPracticeExperiment;
export const PRACTICE_DAILY_TRAINING = __catalog_base.PRACTICE_DAILY_TRAINING;
