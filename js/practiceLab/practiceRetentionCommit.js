...
export async function preparePracticeRetentionReviewChanges({ repository, sessionSummary, reviewDeltas = [], policy = PRACTICE_REVIEW_POLICY_V1 } = {}) {
  const validation = validatePracticeRetentionReviewDeltaBatch(reviewDeltas, { sessionId: sessionSummary?.sessionId, profileId: sessionSummary?.profileId, contextId: sessionSummary?.contextId }, policy);
  ...
  for (const delta of reviewDeltas) {
    ...
    if (!item) { staleReviewDeltaCount += 1; continue; }
    if (item.profileId !== delta.profileId || item.contextId !== delta.contextId || item.entityType !== delta.entityType || item.entityKey !== delta.entityKey) throw validationError("Practice retention delta identity does not match review item", [{ reviewItemId: delta.reviewItemId }]);
    const merged = mergePracticeRetentionReviewDelta(item, delta, policy);
    if (merged.stale) { staleReviewDeltaCount += 1; continue; }
...
export async function commitCompletedPracticeRetentionSession({
  ...
  const existingSession = typeof repository.getSessionSummary === "function" ? await repository.getSessionSummary(sessionSummary.sessionId) : null;
  if (existingSession) {
