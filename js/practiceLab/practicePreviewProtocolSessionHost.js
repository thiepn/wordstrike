...
export async function mountPracticePreviewProtocolSession({root,experimentId,durationMs,onExit=()=>{}}){
  ...
  async function exit(){if(closed)return;closed=true;pulse?.stop();unsubscribe?.();doc.removeEventListener('visibilitychange',visibility);try{if(['active','paused','ready'].includes(engine?.getSnapshot().lifecycleState))await engine.abandon('manual-stop');await engine?.destroy();}finally{dataStore.close();onExit();}}
  ...
  try {
    ...
    unsubscribe=engine.subscribe((snapshot,event)=>{if(event==='completed'){finished=true;pulse?.stop();void engine.complete().then(result=>{if(closed)return;const analysis=result.summary?.trainingQuality;main.replaceChildren(node('h1','Session complete'),node('p',`Observed pattern: ${analysis?.pattern??'insufficient evidence'}`),node('p','This is a within-session descriptive association, not proof of improvement. Sparse or interrupted blocks do not support a conclusion.'),stop);stop.textContent='BACK TO SETUP';stop.focus();}).catch(()=>{status.textContent='Saving failed. End the session and try again.';});}else paint(snapshot);});
