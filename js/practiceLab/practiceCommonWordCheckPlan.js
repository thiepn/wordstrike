...
function buildUnits(words) {
  ...
  return words.map((word, index) => {
    ...
    return { unitId: `breadth-word_${index + 1}`, type: "word", startIndex, endIndex, text: word.lexicalKey, metadata: { commonWords: { lexicalKey: word.lexicalKey, rank: word.rank, band: word.band, ordinal: index + 1 } } };
  });
}
...
export function createPracticeCommonWordCheckPlan({ sessionId, profileId, contextId, formSet } = {}) {
  ...
  const planBinding = { formId: form.formId, formHash: form.formHash, schemaVersion: PRACTICE_COMMON_WORD_CHECK_SCHEMA_VERSION, generatorVersion: PRACTICE_COMMON_WORD_CHECK_GENERATOR_VERSION, referenceVersion: PRACTICE_COMMON_WORD_REFERENCE_VERSION };
  const planHash = hashPracticeContent(JSON.stringify(planBinding));
  const text = form.words.map((word) => word.lexicalKey).join(" ");
  const plan = freezeDeep({
    ...
    wordCount: PRACTICE_COMMON_WORD_CHECK_WORD_COUNT, bandCounts,
  });
