...
export function renderPracticeEvidenceView(root,kind,state,{focus=false,embedded=false}={}) {
  ...
  if(state.status==='loading')body='<p role="status">Loading local evidence…</p>';
    ...
    body=rows.length?`<p>${rows.length} saved sessions in this context.</p><ol>${rows.slice(0,100).map(s=>`<li class="practice-lab-empty-state"><strong>${esc(s.experimentId)}</strong> · ${esc(s.status)}<p>${esc(s.completedAtUtc??s.startedAtUtc??'')}</p><p>${number(s.afterMetrics?.wpm)} WPM · ${number(s.afterMetrics?.firstPassAccuracy??s.afterMetrics?.accuracy)}% accuracy</p></li>`).join('')}</ol>${rows.length>100?'<p>Showing the latest 100 sessions.</p>':''}`:'<h2>No training history</h2><p>Completed and interrupted sessions will appear here after they are saved.</p>';
  }
