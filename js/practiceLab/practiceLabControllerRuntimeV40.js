...
export function createPracticeLabController(options={}) {
  ...
  function renderContent(renderRoot,view,renderOptions={}) {
    ...
    if(view.kind==='full-assessment-detail'&&state.status==='error'){const alert=renderRoot.ownerDocument.createElement('div');alert.className='practice-lab-notice is-warning';alert.setAttribute('role','alert');alert.innerHTML='<strong>Assessment could not load.</strong><p>Your existing Practice data has not been changed.</p><button type="button" data-practice-action="assessment-refresh">TRY AGAIN</button>';renderRoot.querySelector('main')?.append(alert);}
