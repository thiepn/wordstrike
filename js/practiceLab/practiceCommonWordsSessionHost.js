...
const percentPoints = (value) => finite(value) ? `${n(value, 1)}%` : "—";
const ratioPercent = (value) => finite(value) ? `${n(value * 100, 1)}%` : "—";
...
function coverageRows(snapshot) {
  return PRACTICE_COMMON_WORD_BANDS.map((band) => { const row = snapshot?.bands?.[band] ?? {}; return `<tr><th scope="row">${PRACTICE_COMMON_WORD_BAND_LABELS[band]}</th><td>${percentPoints(row.observedPercent)}</td><td>${percentPoints(row.repeatedEvidencePercent)}</td><td>${percentPoints(row.automaticPercent)}</td></tr>`; }).join("");
}
function bandRows(metrics) {
  return PRACTICE_COMMON_WORD_BANDS.map((band) => { const row = metrics?.[band] ?? {}; return `<tr><th scope="row">${PRACTICE_COMMON_WORD_BAND_LABELS[band]}</th><td>${ratioPercent(row.wholeWordFirstPassAccuracy)}</td><td>${n(row.launchResidualMedianMs)} ms</td><td>${n(row.internalResidualMedianMs)} ms</td><td>${ratioPercent(row.launchDisfluencyRate)}</td><td>${ratioPercent(row.internalDisfluencyRate)}</td></tr>`; }).join("");
}
...
export async function mountPracticeCommonWordsSession({ root, session, runtime = null, onExit = () => {}, logger = null, dependencies = {} } = {}) {
  ...
  const focus = () => queueMicrotask(() => focusPracticeSessionInput(root, "[data-common-words-input]"));
