...
function renderResult(root, session, finalResult, breadthSnapshot, abilityState) {
  ...
  unsubscribe = engine.subscribe((snapshot, event) => {
    if (event === "completed") {
      void engine.complete().then(async (result) => {
        ...
        renderResult(root, session, result, breadth ?? session.breadthSnapshotBefore, ability);
