...
export function createPracticeLabController(options = {}) {
  ...
  const loadRecommendations = async () => {
    ...
    const epoch = ++recommendationEpoch; setState({ recommendationStatus: "loading", recommendationErrorCode: null });
    try {
      const result = typeof problemWordsRecommendationLoader === "function" ? await problemWordsRecommendationLoader() : await import("./practiceProblemWordsRecommendationRuntime.js").then((module) => module.loadPracticeProblemWordRecommendations());
      ...
      setState({ recommendationStatus: status, recommendationErrorCode: status === "unavailable" ? result?.errorCode ?? "RECOMMENDATIONS_UNAVAILABLE" : null, recommendations: status === "ready" ? result?.recommendations ?? [] : [] });
      return status === "ready";
    } catch (error) {
      if (epoch !== recommendationEpoch) return false;
      setState({ recommendationStatus: "unavailable", recommendationErrorCode: error?.code ?? "RECOMMENDATIONS_UNAVAILABLE", recommendations: [] }); return false;
    }
  };
  const warningsFor = async (entityKey) => {
    const recommended = state.recommendations.find((entry) => entry.entityKey === entityKey); const warnings = [];
    ...
    try { const result = typeof problemWordsWarningsLoader === "function" ? await problemWordsWarningsLoader({ entityKey }) : await import("./practiceProblemWordsRecommendationRuntime.js").then((module) => module.loadPracticeProblemWordManualWarnings({ entityKey })); return result?.warnings ?? []; } catch { return []; }
  };
  const inspect = async ({ entityKey, targetSource = "manual" } = {}) => {
    const normalized = normalizePracticeProblemWordsManualInput(entityKey, "en"); const epoch = ++inspectEpoch;
    if (!normalized.valid) { setState({ targetValue: normalized.normalizedValue, selectedSource: targetSource, status: "unsupported", reasonCode: normalized.reasonCode, message: normalized.message, availability: null, warnings: [] }, "[data-problem-word-target]"); return false; }
    ...
    try {
      const [availability, warnings] = await Promise.all([registration?.runtime?.inspectTarget?.({ entityKey: normalized.target.entityKey }), warningsFor(normalized.target.entityKey)]);
      ...
      setState({ targetValue: normalized.target.entityKey, selectedSource: targetSource, status, reasonCode: availability?.reasons?.[0] ?? null, availability, warnings }, "[data-problem-word-target]"); return status === "ready";
    } catch (error) { if (epoch !== inspectEpoch) return false; setState({ status: "unavailable", reasonCode: error?.code ?? "TRAINING_CORPUS_NOT_READY", availability: null }, "[data-problem-word-target]"); return false; }
  };
  const prepare = async ({ entityKey, targetSource = "manual" } = {}) => {
    const normalized = normalizePracticeProblemWordsManualInput(entityKey, "en"); if (!normalized.valid) return inspect({ entityKey, targetSource });
    const epoch = ++prepareEpoch; setState({ targetValue: normalized.target.entityKey, selectedSource: targetSource, status: "preparing", reasonCode: null, message: null });
    const registration = experimentRegistry.getRegistration(PROBLEM_WORDS_ID);
    try {
      const prepared = await registration.setupFactory({ entityKey: normalized.target.entityKey, targetSource }); if (!mounted || epoch !== prepareEpoch) return false;
      const session = registration.sessionFactory(prepared); const { mountPracticeProblemWordsSession } = await import("./practiceProblemWordsSessionHost.js"); if (!mounted || epoch !== prepareEpoch) return false;
      sessionHost = await mountPracticeProblemWordsSession({ root, session, logger,
        onExit() { sessionHost = null; if (mounted) setState({ status: "ready", reasonCode: null, message: null }, "[data-problem-word-target]"); },
        onRepeat() { sessionHost = null; if (mounted) void prepare({ entityKey: normalized.target.entityKey, targetSource }); },
      });
      return true;
    } catch (error) {
      if (!mounted || epoch !== prepareEpoch) return false; const code = error?.code ?? "TRAINING_CORPUS_NOT_READY"; const status = code === "UNSUPPORTED_WORD_TARGET" ? "unsupported" : LIMITED_CODES.has(code) ? "limited-content" : "unavailable";
      logger?.warn?.("Problem Words preparation failed", error); setState({ status, reasonCode: code, message: null }, "[data-problem-word-target]"); return false;
    }
  };

  const click = (event) => {
    const target = event.target?.closest?.("[data-practice-action]"); if (!target || !root?.contains?.(target)) return;
    const action = target.dataset.practiceAction;
    if (action === "choose-problem-word") { event.stopPropagation(); void inspect({ entityKey: target.dataset.entityKey, targetSource: target.dataset.targetSource ?? "recommended" }); }
  ...
  return Object.freeze({
    ...
    navigate(...args) { const result = base.navigate(...args); queueMicrotask(afterRoute); return result; },
    back() { if (sessionHost) { void sessionHost.exit(); return true; } const result = base.back(); queueMicrotask(afterRoute); return result; },
    getSnapshot() { const snapshot = base.getSnapshot(); return Object.freeze({ ...snapshot, problemWords: Object.freeze({ status: state.status, targetValue: state.targetValue, recommendationStatus: state.recommendationStatus, recommendationCount: state.recommendations.length, sessionActive: Boolean(sessionHost) }) }); },
    subscribe(listener) { return base.subscribe(listener); },
    unmount() { mounted = false; inspectEpoch += 1; prepareEpoch += 1; recommendationEpoch += 1; root?.removeEventListener?.("click", click, true); root?.removeEventListener?.("input", input, true); if (sessionHost) { void sessionHost.exit(); sessionHost = null; } return base.unmount(); },
  });
}
