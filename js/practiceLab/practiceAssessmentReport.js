import {
  ...
  PRACTICE_ASSESSMENT_REPORT_VERSION,
...
function masteryCounts(masterySnapshot) {
  const defaults = {
    stages: { unmeasured: 0, learning: 0, acquired: 0, transferred: 0, robust: 0, retained: 0 },
    automaticity: { developing: 0, emerging: 0, established: 0, strong: 0 },
  };
  const canonicalStages = masterySnapshot?.counts?.stageCounts;
  const canonicalAutomaticity = masterySnapshot?.counts?.automaticityCounts;
  if (canonicalStages || canonicalAutomaticity) {
    return {
      stages: { ...defaults.stages, ...(canonicalStages ?? {}) },
      automaticity: {
        ...defaults.automaticity,
        ...(canonicalAutomaticity ?? {}),
      },
    };
  }
  for (const entry of masterySnapshot?.entities ?? masterySnapshot?.items ?? []) {
    const key = String(entry.stage ?? "unmeasured").toLowerCase();
    if (Object.hasOwn(defaults.stages, key)) defaults.stages[key] += 1;
    const auto = String(entry.automaticity?.status ?? entry.automaticityStatus ?? "").toLowerCase();
    if (Object.hasOwn(defaults.automaticity, auto)) defaults.automaticity[auto] += 1;
  }
  return defaults;
}

function limiterSummaries(snapshot) {
  if (!snapshot) return [];
  const primary = new Set(snapshot.primaryLimiterIds ?? []);
  const candidates = Array.isArray(snapshot.candidates) ? snapshot.candidates : [];
  const ordered = [
    ...candidates.filter((item) => primary.has(item.statId)),
    ...candidates.filter((item) => !primary.has(item.statId)),
  ];
  return ordered.slice(0, PRACTICE_ASSESSMENT_LIMITS.maximumStoredLimiterSummaries).map((item) => Object.freeze({
    statId: item.statId ?? null,
    entityType: item.entityType ?? null,
    entityKey: item.entityKey ?? null,
    phenotype: item.primaryPhenotype ?? item.phenotype ?? null,
    status: item.status ?? null,
    weaknessScore: item.weaknessScore ?? null,
    impactScore: item.impact?.impactScore ?? item.impactScore ?? null,
    priorityScore: item.priorityScore ?? null,
    evidenceConfidenceLevel: item.evidenceConfidenceLevel ?? item.confidenceLevel ?? null,
    hierarchyStatus: item.hierarchy?.status ?? item.hierarchyStatus ?? null,
  }));
}

function abilitySummary(abilityState, benchmarkEval) {
  if (!abilityState) return null;
  const estimate = abilityState.estimate ?? abilityState;
  return Object.freeze({
    meanLogWpm: Number.isFinite(estimate.meanLogWpm) ? estimate.meanLogWpm : null,
    varianceLogWpm: Number.isFinite(estimate.varianceLogWpm) ? estimate.varianceLogWpm : null,
    estimateWpm: estimate.estimateWpm ?? null,
    interval95LowerWpm: estimate.interval95LowerWpm ?? null,
    interval95UpperWpm: estimate.interval95UpperWpm ?? null,
    confidenceLevel: estimate.confidenceLevel ?? null,
    abilityUpdatedByAssessment: benchmarkEval?.abilityEligible === true || benchmarkEval?.evidenceAdmission?.ability === true,
  });
}
...
export function buildPracticeAssessmentReport({ run, limiterSnapshot = null, abilityState = null, masterySnapshot = null } = {}) {
  ...
  const benchmarkFreshness = benchmarkEval?.freshnessStatus ?? benchmarkEval?.freshness ?? null;
  const transferFreshness = transferEval?.freshnessStatus ?? transferEval?.freshness ?? null;
  const nonstandard = run.integrityStatus === "nonstandard" || benchmarkFreshness === "repeat";
  const reportStatus = run.integrityStatus === "invalid" ? "invalid" : invalidBlocks.length ? "partial" : "complete";
  const limiters = limiterSummaries(limiterSnapshot);
  ...
  return Object.freeze({
    ...
    blueprintVersion: PRACTICE_ASSESSMENT_BLUEPRINT_VERSION,
    depth: run.depth,
    ...
    blockSummary: Object.freeze(run.blocks.map((block) => Object.freeze({
      blockId: block.blockId,
      status: block.status,
      sessionId: block.childSessionId,
      durationMs: block.durationMs,
      freshness: block.result?.diagnosticFreshness ?? block.result?.evaluationSummary?.freshnessStatus ?? block.result?.evaluationSummary?.freshness ?? null,
    }))),
    generalPerformance: Object.freeze({
      benchmark: benchmarkMetrics ? Object.freeze({
        wpm: benchmarkMetrics.wpm,
        rawWpm: benchmarkMetrics.rawWpm,
        adjustedWpm: benchmarkEval?.adjustedWpm ?? null,
        accuracy: benchmarkMetrics.accuracy,
        measurementSigmaLog: benchmarkEval?.measurementSigmaLog ?? null,
        freshness: benchmarkFreshness,
        comparabilityClass: benchmarkEval?.comparabilityClass ?? null,
      }) : null,
      coldNaturalAbility: abilitySummary(abilityState, benchmarkEval),
    }),
    control: buildControl(run.blocks),
    diagnosticCoverage: Object.freeze({ blueprintVersion: PRACTICE_ASSESSMENT_BLUEPRINT_VERSION, blocks: Object.freeze(clone(coverage)) }),
    limiterSnapshot: Object.freeze(limiters),
    masterySnapshot: Object.freeze(mastery),
    transfer: run.depth === "deep" ? Object.freeze({
      available: Boolean(transferMetrics),
      freshness: transferFreshness,
      integrity: transferEval?.integrityStatus ?? transferEval?.status ?? null,
      wpm: transferMetrics?.wpm ?? null,
      adjustedWpm: transferEval?.adjustedWpm ?? null,
      accuracy: transferMetrics?.accuracy ?? null,
      validTransferEvidenceEntityCount: transferEval?.validTransferEvidenceEntityCount ?? null,
      adjustedWpmDelta: Number.isFinite(transferEval?.adjustedWpm) && Number.isFinite(benchmarkEval?.adjustedWpm) ? transferEval.adjustedWpm - benchmarkEval.adjustedWpm : null,
      deltaLabel: "descriptive",
    }) : Object.freeze({
      available: false,
      freshness: null,
      integrity: null,
      ...
      adjustedWpm: null,
      accuracy: null,
      validTransferEvidenceEntityCount: null,
      adjustedWpmDelta: null,
      deltaLabel: "not-measured",
    }),
