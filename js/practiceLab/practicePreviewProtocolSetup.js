export function renderPracticePreviewProtocolSetup(root,experimentId){
  ...
  main.append(node('p','These are descriptive practice protocols. Read-Ahead does not measure eye movements, and neither protocol establishes a causal benefit. Leaving the page ends the session.'));
  ...
  const start=node('button','START SESSION');start.type='button';start.dataset.practiceAction='start-preview-protocol';start.dataset.experimentId=experimentId;main.append(start);root.replaceChildren(section);const heading=root.querySelector('h1');heading.tabIndex=-1;heading.focus({preventScroll:true});
}
