...
export function renderPracticeProblemWordsSessionSnapshot(root, { contentPlan, snapshot }) {
  ...
  renderPracticeSessionMarkup(root, `<section class="screen practice-lab-screen practice-weak-key-session practice-problem-word-session" data-practice-view="problem-words-session"><div class="practice-lab-shell">
...
export function renderPracticeProblemWordsResult(root, finalResult) {
  ...
  renderPracticeSessionMarkup(root, `<section class="screen practice-lab-screen practice-weak-key-results" data-practice-view="problem-words-result"><div class="practice-lab-shell"><main class="practice-lab-detail"><div class="eyebrow">Problem Words complete</div><h1>Word: ${escapeHtml(target)}</h1><p class="practice-lab-lead">Baseline → Check execution delta: ${signed(immediate)} quality points.</p>${resultGrid(before, after)}<section class="practice-lab-empty-state"><h2>Session deltas</h2><dl><div><dt>Start residual change</dt><dd>${signed(launchDelta, " ms")}</dd></div><div><dt>Inside-word residual change</dt><dd>${signed(internalDelta, " ms")}</dd></div><div><dt>Dose completed</dt><dd>1.0 · 15 direct word opportunities</dd></div></dl><p>Timing residuals are milliseconds relative to expected context. Negative residuals are faster than expected; a negative change means the Check moved faster relative to context.</p></section><div class="practice-lab-notice" role="note">This compares your Baseline and Check inside this practice session. Long-term learning, transfer, and retention require later evidence.</div><div class="practice-weak-key-result-actions"><button type="button" data-problem-words-session-action="repeat">PRACTICE AGAIN</button><button type="button" data-problem-words-session-action="finish">BACK TO PROBLEM WORDS</button></div></main></div></section>`);
}
...
export async function mountPracticeProblemWordsSession({ root, session, onExit = () => {}, onRepeat = () => {}, logger = null, dependencies = {} } = {}) {
  ...
  const focus = () => queueMicrotask(() => focusPracticeSessionInput(root, "[data-problem-words-input]"));
  ...
  const finish = async (repeat = false) => { if (closed) return; closed = true; unsubscribe?.(); root.removeEventListener("beforeinput", beforeInput); root.removeEventListener("keydown", keyDown); root.removeEventListener("click", click); globalThis.document?.removeEventListener?.("visibilitychange", visibilityChange); try { await engine.destroy(); } catch {} try { dataStore.close?.(); } catch {} (repeat ? onRepeat : onExit)(finalResult); };
  ...
  const click = (event) => { const target = event.target?.closest?.(ACTION_SELECTOR); if (!target) return; const action = target.dataset.problemWordsSessionAction; if (action === "pause") void engine.pause("manual"); else if (action === "resume") void engine.resume().then(() => focusPracticeSessionInput(root, "[data-practice-session-capture]", { force: true })); else if (action === "abandon") void engine.abandon("manual-stop").then(() => finish(false)).catch(() => finish(false)); else if (action === "repeat") void finish(true); else if (action === "finish") void finish(false); };
  const visibilityChange = () => { const state = globalThis.document?.visibilityState; if (state === "hidden" || state === "visible") void engine.handleVisibilityState(state).catch((error) => logger?.warn?.("Problem Words visibility transition failed", error)); };
