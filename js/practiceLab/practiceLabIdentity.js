...
function homeMarkup(view) {
  ...
  return `<section class="screen practice-lab-screen pl-studio-home" data-practice-view="home" data-lab-identity="studio"><div class="practice-lab-shell">
    <header class="pl-topbar"><div class="pl-wordmark"><span class="pl-mark" aria-hidden="true">W<span>/</span></span><strong>WORDSTRIKE</strong><span class="pl-topbar-divider" aria-hidden="true">/</span><span>TRAINING STUDIO</span></div><div class="pl-topbar-actions">${view.dataManagementAvailable ? '<button type="button" data-practice-action="manage-data" aria-label="Manage Practice Lab data">DATA</button>' : ''}<button type="button" class="practice-lab-help" data-practice-action="help" aria-label="Practice Lab help"${view.helpAvailable ? '' : ' disabled aria-disabled="true"'}>HELP</button><button type="button" data-practice-action="exit" aria-label="Exit Practice Lab">EXIT LAB <span aria-hidden="true">↗</span></button></div></header>
