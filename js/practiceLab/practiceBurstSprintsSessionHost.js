...
function renderResult(root, artifact, interrupted = false) {
  ...
  shell(root, `<main class="practice-lab-detail"><div class="eyebrow">Burst Sprints ${interrupted ? "stopped" : "complete"}</div><h1>${interrupted ? "Incomplete sprint protocol" : "Burst diagnostic"}</h1><dl class="practice-real-text-result-grid"><div><dt>Best observed sprint</dt><dd>${num(artifact?.bestObservedSprintWpm)} WPM</dd></div><div><dt>Session burst estimate</dt><dd>${num(artifact?.sessionBurstEstimateWpm)} WPM</dd></div><div><dt>Current burst ability</dt><dd>${num(artifact?.currentBurstAbilityWpm)} WPM</dd></div><div><dt>Burst reserve</dt><dd>${num(artifact?.pl14BurstReserveWpm)} WPM</dd></div><div><dt>Eligible sprints</dt><dd>${artifact?.eligibleSprintCount ?? 0} / 6</dd></div></dl><table><thead><tr><th>Sprint</th><th>Gross (WPM)</th><th>Burst effective (WPM)</th><th>First-pass accuracy</th><th>Validity</th><th>Estimator</th></tr></thead><tbody>${rows(artifact?.sprints, selected)}</tbody></table><div class="practice-lab-notice"><p>${esc(artifact?.interpretation ?? "No burst estimate admitted.")}</p><p>The 75% floor is a measurement-validity rule, not a recommended typing accuracy target.</p></div><button type="button" data-burst-session-action="finish">BACK TO BURST SPRINTS</button></main>`, "burst-sprints-result");
}
