...
export function renderPracticePhysicalTelemetryPanel({ availability, snapshot, hasStoredData = false } = {}) {
  ...
  return `<section class="practice-physical-telemetry" data-practice-physical-telemetry data-diagnostics-version="${PRACTICE_PHYSICAL_DIAGNOSTICS_VERSION}">
    ...
    <h3>Physical Codes</h3><table><thead><tr><th scope="col">Physical code</th><th scope="col">Events</th><th scope="col">Observed misstrike origins</th><th scope="col">Median residual</th><th scope="col">Disfluency</th><th scope="col">Evidence</th></tr></thead><tbody>${rows(snapshot?.keys, "key")}</tbody></table>
