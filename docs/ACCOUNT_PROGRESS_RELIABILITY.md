...
- Results hover handlers rebuilt entire screens. Selection now changes classes in place. Data-only screen refreshes preserve scroll, expanded and collapsed details, result tabs and text editing state. Coalesced updates retain the original scroll position rather than a transient collapsed layout; fresh user interaction cancels restoration. There are no repeated scroll timers.
...
Campaign bests and placement use high-water merges. Aggregate counters use per-runtime writer components; retrying the same upload cannot add a run again. Pre-migration totals are conservatively max-merged because the old aggregates do not establish which runs overlap. Detailed recent-session history retains the application's existing bounded history; this is not an unlimited keystroke archive. The separate Practice Lab drill database is not included in this profile snapshot; shared mode statistics and records are included.
...
- `tests/screen-view-state.test.js`: coalesced renders, closed details, user interaction and navigation cancellation.
...
The reconstructed candidate passed all 436 JavaScript test files, all 26 new Chromium/Firefox browser checks, Campaign recovery in both browsers, and all nine existing non-Practice browser scenarios. Evidence: https://github.com/thiepn/wordstrike/actions/runs/35795167220. The permanent account/progress workflow repeats the targeted checks on relevant pull requests and main-branch changes; the existing Tests workflow runs the complete JavaScript suite.
