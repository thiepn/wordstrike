...
const replaceOnce = (source, search, replacement, label) => {
  ...
  if (index < 0) throw new Error(`PL11 final patch missing anchor: ${label}`);
  if (source.indexOf(search, index + search.length) >= 0) throw new Error(`PL11 final patch ambiguous anchor: ${label}`);
...
const replaceRegex = (source, regex, replacement, label) => {
  ...
  if (matches.length !== 1) throw new Error(`PL11 final patch expected one ${label}, found ${matches.length}`);
...
`  errorTrackerSnapshot = null,
  ...
  startedAtUtc = null,`, "checkpoint skill snapshot arg");
  source = replaceOnce(source,
...
console.log("PL11 final integration patch applied");
