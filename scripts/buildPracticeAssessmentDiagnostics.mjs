...
import { scorePracticeTextTypability, PRACTICE_TYPABILITY_WEIGHTS_V1 } from '../js/practiceLab/practiceTypabilityModel.js';
...
const reference=JSON.parse(await readFile(path.join(root,'data/practice/models/en-v1/typability-v1.reference.json'),'utf8'));
const frequencyProvider=createPracticeReferenceFrequencyProvider(JSON.parse(await readFile(path.join(root,'data/practice/provenance/frequency/en-v1.frequency.json'),'utf8')));
...
const matchPair=(forms)=>{
 ...
 const names=Object.keys(PRACTICE_TYPABILITY_WEIGHTS_V1).filter(k=>Number.isFinite(a.standardizedFeatures[k])&&Number.isFinite(b.standardizedFeatures[k]));
 const weight=names.reduce((s,k)=>s+PRACTICE_TYPABILITY_WEIGHTS_V1[k],0);
 const rms=weight?Math.sqrt(names.reduce((s,k)=>s+PRACTICE_TYPABILITY_WEIGHTS_V1[k]*(a.standardizedFeatures[k]-b.standardizedFeatures[k])**2,0)/weight):Infinity;
 return rms<=p.maximumWeightedRmsFeatureDistance && da<=p.maximumDifficultyIndexSpread && pa<=p.maximumRelativePercentileSpread && ld<=p.maximumLengthDeviationRatio;
};
const formSets=[];
for(const block of blueprint.blocks){
 ...
 for(const item of candidates) { if(!families.has(item.familyId)) families.set(item.familyId,[]); families.get(item.familyId).push(item); }
 const familySeen=new Set(); const forms=[];
 for(const segments of families.values()){
   segments.sort((a,b)=>a.contentId.localeCompare(b.contentId));
   const text=segments.map(i=>i.text).join(' ');
   const item={...segments[0],text,contentHash:'sha256-'+createHash('sha256').update(text).digest('hex')};
   const features=extractPracticeTextDifficultyFeatures({text,language:corpus.language,frequencyProvider});
   const compositeModel={features,textDifficulty:scorePracticeTextTypability({features,reference,language:corpus.language})};
   if(familySeen.has(item.familyId)) continue; const mi=segments.every(i=>byModel.has(i.contentId))?compositeModel:null; if(!mi){reasons.push(`${item.contentId}:missing-model`);continue;}
   ...
   familySeen.add(item.familyId); forms.push({formId:`${block.blockId}-form-${String(forms.length+1).padStart(2,'0')}`,contentIds:segments.map(i=>i.contentId),segmentHashes:segments.map(i=>i.contentHash),segmentGraphemes:segments.map(i=>[...i.text].length),joiner:" ",familyId:item.familyId,sourceId:item.sourceId,partition:'diagnostic',graphemeCount:graphemes,contentHash:item.contentHash,difficultyIndex:mi.textDifficulty.difficultyIndex,relativeDifficultyPercentile:mi.textDifficulty.relativeDifficultyPercentile,standardizedFeatures:mi.textDifficulty.standardizedFeatures,availableModelWeight:mi.textDifficulty.availableModelWeight});
 }
