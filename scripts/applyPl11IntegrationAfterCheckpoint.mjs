...
const replaceOnce = (source, search, replacement, label) => {
  ...
  if (index < 0) throw new Error(`PL11 session patch missing anchor: ${label}`);
  if (source.indexOf(search, index + search.length) >= 0) throw new Error(`PL11 session patch ambiguous anchor: ${label}`);
...
const replaceRegex = (source, regex, replacement, label) => {
  ...
  if (matches.length !== 1) throw new Error(`PL11 session patch expected one ${label}, found ${matches.length}`);
...
`      errorTrackerSnapshot: errorTracker.checkpointSnapshot({ contentHash: contentPlan.contentHash, cursorIndex: typingState.cursorIndex }),
      ...
      startedAtUtc,`, "checkpoint engine tracker snapshot");
  source = replaceOnce(source,
...
console.log("PL11 session/repository integration patch applied");
